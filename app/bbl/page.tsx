...
export default function BBLPage() {
  ...
  const allMatches = useMemo(() => (bblData as any).matches as LocalMatch[], []);
        ...
        {activeTab === 'spelschema' && (
          <>
            ...
            <div className="bg-primary text-white px-4 py-3 mt-6 mb-0">
              <h2 className="text-lg font-bold uppercase">BBL SPELSCHEMA</h2>
            </div>
            {sortedMatches.length === 0 ? (
              ...
              <div className="bg-white">
                {sortedMatches.map(match => (
                  <MatchCard key={match.id} id={match.id} league="BBL" home={match.home} away={match.away} time={match.time} date={match.date} venue={match.venue} broadcasters={match.broadcasters} />
                ))}
              </div>
            )}
        ...
        {activeTab === 'tabell' && (
          <div className="mt-6">
            <div className="bg-primary text-white px-4 py-3 mb-0">
              <h2 className="text-lg font-bold uppercase">BBL TABELL 2025</h2>
            </div>
            <LeagueStandings league="bbl" />
          </div>
        )}
      </div>
    </div>
