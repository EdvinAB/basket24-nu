...
import serbianData from '@/lib/data/serbian-super-league-2025-2026.json';
...
export default function SerbianSuperLeaguePage() {
  const [activeTab, setActiveTab] = useState<'spelschema' | 'tabell'>('spelschema');
  const [selectedDate, setSelectedDate] = useState<string>(() => new Date().toISOString().split('T')[0]);

  const allMatches = useMemo(() => (serbianData as any).matches as LocalMatch[], []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <LeagueHero league="Serbian Super League" />
        ...
        {activeTab === 'spelschema' && (
          <>
            ...
            <div className="bg-primary text-white px-4 py-3 mt-6 mb-0">
              <h2 className="text-lg font-bold uppercase">SERBIAN SUPER LEAGUE SPELSCHEMA</h2>
            </div>
            {sortedMatches.length === 0 ? (
              ...
              <div className="bg-white">
                {sortedMatches.map(match => (
                  <MatchCard key={match.id} id={match.id} league="Serbian Super League" home={match.home} away={match.away} time={match.time} date={match.date} venue={match.venue} broadcasters={match.broadcasters} />
                ))}
              </div>
            )}
        ...
        {activeTab === 'tabell' && (
          <div className="mt-6">
            <div className="bg-primary text-white px-4 py-3 mb-0">
              <h2 className="text-lg font-bold uppercase">SERBIAN SUPER LEAGUE TABELL 2025</h2>
            </div>
            <LeagueStandings league="serbian-super-league" />
          </div>
        )}
      </div>
    </div>
