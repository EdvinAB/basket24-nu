...
export default function EuroLeaguePage() {
  ...
  const allMatches = useMemo(() => (euroleagueData as any).matches as LocalMatch[], []);
            ...
            {sortedMatches.length === 0 ? (
              ...
              <div className="bg-white">
                {sortedMatches.map(match => (
                  <MatchCard key={match.id} id={match.id} league="EuroLeague" home={match.home} away={match.away} time={match.time} date={match.date} venue={match.venue} broadcasters={match.broadcasters} />
                ))}
              </div>
            )}
        ...
        {activeTab === 'tabell' && (
          <div className="mt-6">
            <div className="bg-primary text-white px-4 py-3 mb-0">
              <h2 className="text-lg font-bold uppercase">EUROLEAGUE TABELL 2025</h2>
            </div>
            <LeagueStandings league="euroleague" />
          </div>
        )}
      </div>
    </div>
